        ...
        {panel === "create" && (
          <aside className="user-editor-card">
            ...
            <form className="user-form" onSubmit={submitCreate}>
              ...
              <label><span>ชื่อผู้ใช้งาน</span><input required value={createForm.fullName} onChange={(event) => setCreateForm((current) => ({ ...current, fullName: event.target.value }))} /></label>
              <label><span>อีเมล</span><input required type="email" value={createForm.email} onChange={(event) => setCreateForm((current) => ({ ...current, email: event.target.value }))} /></label>
              <label><span>บทบาท</span><select value={createForm.role} onChange={(event) => setCreateForm((current) => ({ ...current, role: event.target.value }))}>{(state.meta?.roles || []).filter((role) => canManageAdmins || !["owner", "admin"].includes(role)).map((role) => <option key={role} value={role}>{roleLabels[role] || role}</option>)}</select></label>
              {createForm.mode === "temporary_password" && <label><span>รหัสผ่านชั่วคราว</span><input required type="password" autoComplete="new-password" value={createForm.temporaryPassword} onChange={(event) => setCreateForm((current) => ({ ...current, temporaryPassword: event.target.value }))} /><small>อย่างน้อย 12 ตัว พร้อมตัวพิมพ์เล็ก พิมพ์ใหญ่ ตัวเลข และสัญลักษณ์</small></label>}
