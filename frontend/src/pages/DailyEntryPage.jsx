    ...
    <>
      ...
      {selectedCategory && (
        <>
          ...
          <section className="daily-table-card unified-entry-card">
            <div className="daily-grid-heading">
              <div><p className="eyebrow">Daily Table</p><h2>ตารางข้อมูลรายวัน</h2><p>กดเลือกแถวเพื่อเลื่อนไปแก้ไขช่องกรอกของวันนั้น</p></div>
              <span>{monthLabelThai(month)}</span>
            </div>
              ...
              <table className="daily-entry-table">
                ...
                <tbody>
                  {days.map((item, index) => {
                    ...
                    return (
                      <tr key={item.date} className={`${selectedDay === item.day ? "selected" : ""} ${item.future ? "future-row" : ""}`} onClick={() => selectDailyRow(index)}>
                        <td>วันที่ {item.day}</td>
                        <td>{item.weekday}</td>
                        <td>{filled ? item.value : "-"}</td>
                        <td>{selectedCategory.unit}</td>
                        <td>{item.future ? "ยังไม่ถึงวันที่บันทึก" : filled ? "มีข้อมูล" : "ยังไม่กรอก"}</td>
                      </tr>
                    );
                  })}
                </tbody>
