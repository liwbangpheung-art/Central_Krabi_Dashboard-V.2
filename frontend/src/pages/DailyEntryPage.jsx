...
export function DailyEntryPage({
  ...
  return (
    <>
      ...
      {fixedModule && (
        <section className="fixed-task-context-strip">
          <span>{selectedCategory?.name_th || moduleLabel[module]}</span>
          <strong>{monthLabelThai(month)}</strong>
          <button className="text-button" type="button" onClick={reloadEntries} disabled={!categoryId || entryState.loading || entryState.saving}>
            {entryState.loading ? "กำลังโหลด..." : "รีเฟรช"}
          </button>
        </section>
      )}
      ...
      {selectedCategory && (
        <>
          ...
          <section className="daily-action-panel unified-entry-card unified-action-card">
            <div>
              <p className="eyebrow">{fixedModule ? "บันทึกข้อมูล" : "เครื่องมือกรอกข้อมูล"}</p>
            ...
            <div className={`daily-action-buttons operator-action-bar ${fixedModule ? "daily-action-buttons-compact" : ""}`}>
                ...
                <div className="advanced-menu">
                  ...
                  {advancedActionsOpen && (
                    <div className="advanced-menu-panel" role="menu">
                      <button type="button" onClick={() => importInputRef.current?.click()} disabled={!canImport || !canWrite || entryState.loading || entryState.saving}>นำเข้า CSV</button>
                      <button type="button" onClick={exportDailyCsv} disabled={!selectedCategory || entryState.loading || entryState.saving}>ส่งออก CSV</button>
              ...
              <button className="primary-button compact" type="button" onClick={saveMonth} disabled={!canWrite || entryState.loading || entryState.saving}>
                {entryState.saving ? "กำลังบันทึก..." : "บันทึกข้อมูล"}
              </button>
            </div>
          </section>
