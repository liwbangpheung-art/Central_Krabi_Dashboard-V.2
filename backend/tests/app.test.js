...
function createFakeSupabase({ validToken = true, role = "admin", readyError = null, profileExists = true, active = true } = {}) {
  ...
  const db = {
    ...
    profiles: profileExists ? [{ id: "user-1", email: "admin@example.com", full_name: "Admin", role, active }] : [],
    master_categories: [
  ...
  function execute(table, state, mode = "many") {
    if (readyError && ["app_settings", "master_categories", "scrap_price_history", "daily_entries", "daily_entry_month_logs", "scrap_sales"].includes(table) && state.operation === "select") {
...
describe("Phase 4 API", () => {
  ...
  it("GET /ready returns 503 when Phase tables are missing", async () => {
